...
//Notify users when mixing HTTP/HTTPS passwords
    ...
    onAttach: function(worker) {
        worker.port.on('blur', function(msg) {
            ...
            for (var password in loginsTable) {
                if (passwordSimilarityCheck(password, newPassword)) {
                    // Ensure that this host doesn't already have this password
                    var httpURLs = loginsTable[password].filter(function(x) { return x.substr(0,7) == 'http://' || x.substr(0,8) == 'https://'; });
                    var hostnames = httpURLs.map(function(x) { return url.URL(x).host; });
